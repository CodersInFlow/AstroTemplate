      ...
      <div className="max-w-[840px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
        <article className="py-8">
          ...
          <header className="mb-12">
            ...
            <h1 className="text-[42px] leading-[1.2] font-serif font-bold mb-4 text-blog-text-primary">
              {post.title}
            </h1>
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-blog-text-primary text-base">
                    {post.author_data?.name || 'Anonymous'}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-sm text-blog-text-secondary">
      ...
      {toc.length > 0 && (
        <aside className="hidden lg:block fixed right-2 lg:right-4 xl:right-8 top-24 w-48 lg:w-56 xl:w-64">
          <div className="bg-blog-card-bg border border-blog-border rounded-lg p-4 lg:p-5 xl:p-6">
            <h3 className="text-xs font-semibold mb-3 lg:mb-4 text-blog-text-secondary uppercase tracking-wider">On this page</h3>
            <nav className="space-y-1">
              {toc.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToHeading(item.id)}
                  className={`block text-left w-full py-1 text-blog-text-muted hover:text-blog-text-primary transition-colors text-xs lg:text-sm ${
